...
%% Last-Updated: 2019-01-11T22:49:46+0100
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Report-no: ***
\newif\ifarxiv
\arxivfalse
...
\newcommand*{\lcond}[1][]{\mathpunct{#1|\ }}%conditional sign (in probabilities)
%\newcommand*{\bigcond}{\mathpunct{\big|\ }}%conditional sign (in probabilities)
%\newcommand*{\lbigcond}{\mathpunct{\big|\ }}%conditional sign (in probabilities)
%\newcommand*{\+}{\lor}
%\renewcommand{\*}{\land}
...
may enter the mathematical formulation of our uncertainty such situations,
...
physicists must choose a particular physical theory \citep[a problem
...
problems are transformed into metaphysical questions like \enquote{is this
...
\citep{truesdell1968,kline1985,brush1976c_r1986,brush1976d_r1999,whittaker1910_r1951,whittaker1953}.
...
the full population. Many studies face this kind question in a dichotomous
way: \enquote{Is this particular \snp\ associated with that disease? yes or
  no?}. A first problem with this question is the vague term
\enquote{association}; this vagueness doesn't disappear just because we use
technical terms like \enquote{linkage}. But, whatever we might mean, the
problem is not dichotomous: there is a continuum of differences between the
frequencies of the disease conditional on the variants of all \snp s. The
questions we need to answer are: how do we want to use these conditional
frequencies? what choices do we need to make? Suppose that the idea is to
examine a person's \snp s in order to diagnose the disease, in an automated
way. However, the cost and time involved allow us to check, say, only one
\snp. Our decision problem is to choose the \snp\ to be used in the
automated diagnostics. Assume for simplicity that the candidate \snp s have
only two variants each. The gain in using a particular \snp\ for
diagnostics is proportional to the absolute difference
$\abs{\incr f} \in \clcl{0,1}$ between the frequencies, in the full
population, of the disease conditional on the two variants. If
...
$\abs{\incr f}^2$, and that our \dob\ about $\abs{\incr f}$ given some
sample study with data $\yD$ and background information $\yI$ is
...
$\expe[\big]{\abs{\incr f}^2 \lcond[\big] \yD, \yI}$.
