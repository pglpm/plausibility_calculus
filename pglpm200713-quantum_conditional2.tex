...
%% Last-Updated: 2020-07-14T15:11:50+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
theorem for sufficient statistics,]{koopman1936,pitman1936} discussed the
falsity of such claims already in 1957. The introduction to his work
\parencite{koopman1957} is very clear:
...
\begin{enumerate}[para,label=(\roman*)]
\item\label{item:q_details} It \emph{does} matter whether many photons are
  ...
  operator (which define the photon state) must be specified.
  ...
  conditional-probability rule. The same rule can be applied in the
  analogous quantum situation \parencite[see \eg][]{barnettetal2003}.
...
\item\label{item:q_prob_freq} Owing to the point above, it is important not
  ...
  \amp\ Yao instead do. For example, in some setups we can have a detection
  ...
  the long-run joint density of detections $f(y)$ \parentext{see \eg\ the
    phenomena of higher-order coherence and bunching in the references
    above}. The rules of the probability calculus also apply in such
  situations. For example, we can guess the position of the first photon
  detection given the second from
  $\pf(y_{1} \| y_{2}) \propto \pf(y_{2} \| y_{1})\;\pf(y_{1})$.
  
\item\label{item:q_slitclosed_vs_detect} The long-run \emph{frequency}
  ...
  calculus. This equality is similar to \eqn~\eqref{eq:cross_combine_same}
...
\item In the setup with one or both slits open, the quantity
  ...
  Bohmian mechanics \citep{berndletal1995,duerretal2004,valentinietal2005}
  (which includes as hidden variables the positions of the particles and
  the spatial configuration of a so-called pilot wave). But then note that,
  first, such theory gives experimental predictions in complete agreement
  with standard quantum theory; and second, the rules of probability theory
  are again used without violations in this theory (\eg\ the relation
  between joint and conditional distributions of the particles' positions,
  and so on).
\end{enumerate}

I may add that the idea and parlance of \enquote{photons passing through
  slits} are used today only out of tradition a bit poetically maybe. The
technical parlance, as used routinely in quantum-optics labs for example
\parencite{leonhardt1997,bachoretal1998_r2004} has a very different
underlying picture. The basic \enquote{system} in a quantum-optics
experiment is not the photon, but the modes of the field-configuration
operator. \enquote{Photons} denote states of such field. 
