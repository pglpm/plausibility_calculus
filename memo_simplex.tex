...
%% Last-Updated: 2019-01-14T00:19:33+0100
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Report-no: ***
\newif\ifarxiv
\arxivfalse
...
\newcommand*{\propertitle}{Memos on measures and metrics on simplices}
\newcommand*{\pdftitle}{\propertitle}
\newcommand*{\headtitle}{Memos on simplices}
...
\newcommand*{\ysum}{\mathop{\textstyle\sum}}
%@@@@@@@@@@ new macros end @@@@@@@@@@
...
\begin{abstract}\labelsep 0pt%
  \noindent Some memos on measures and metrics on simplices.
...
Consider $N+1$ mutually exclusive and exhaustive propositions. The belief
distributions and the relative-frequency distributions for them form an
$N$-dimensional simplex. Label the propositions $\set{0,\dotsc,N}$, denote
a distribution by $(q_0, \dotsc, q_N) \defs \yq$. In the rest of this memo
it's always implicitly assumed that $q_i\ge 0$, also in the integration
domains. Denote
\begin{equation}
  \label{eq:def_orthant}
  \ysim_N \defd \set{(x_1,\dotsc,x_N) \suchthat x_i\ge 0, \tsum_i x_i \le 1},
\end{equation}
which is the $(N+1)$-simplex asymmetrically embedded in $\RR^{N+1}$. As
basic volume element we can take either
\begin{gather}
...
\end{gather}
The latter leads to more symmetric formulae. The two volume elements are
...
\textcite[\sect~18.10]{jaynes1994_r2003}. Let's denote either measure by
$\di\yq$. In integrations, when~\eqref{eq:vol_element_1N} is intended, any
$q_0$ that appears in the integral must be understood as
$1-\sum_{i=1}^N q_i$.

The $N$-simplex has a natural convex structure. Thus the ratio of two
$N$-volumes is well-defined. There's only one measure that gives
\begin{enumerate*}[label=(\alph*)]
\item the same \dob\ to any two $N$-volumes having unit ratio,
\item unit \dob\ to the full simplex:
\end{enumerate*}
\begin{equation}
