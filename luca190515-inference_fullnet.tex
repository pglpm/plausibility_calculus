...
%% Last-Updated: 2019-10-29T17:11:00+0100
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
\item $A\in\set{0,1,\dotsc,N}$: total activity of the full network.

\item $A_{t}\in\set{0,1,\dotsc,N}$: activity of the full network at bin $t$.
...
\item $a\in\set{0,1,\dotsc,n}$: total activity of the sample.

\item $a_{t}\in\set{0,1,\dotsc,n}$: activity of the sample at bin $t$.
...
We want $\pf(\yF \| \yas,\yI)$. This can be obtained as the marginal of
$\pf(\yJ \| \yas, \yI)$. If this probability distribution is represented by
a set of Monte Carlo samples $\set{\yJ^{(i)}}$, then
$\set{\yF^{(i)}\equiv\sum_{a}J^{(i)}_{a\,A}}$ are automatically samples of
$\pf(\yF \| \yas,\yI)$.
...
maximum-entropy method in this particular application. From this assumption
...
Now let's derive $\pf(\yAs \| \yI)$ in formula~\eqref{eq:bayes_sequences}.
We assume that the value of this probability has the same value for all
sequences $\yAs$ having the same frequency distribution $\yF(\yAs)$; that
is, it functionally depends on $\yAs$ only through $\yF(\yAs)$. More
specifically we assume it can be written hierarchically as
\begin{equation}
...
further interpreted (if we can solve the integral analytically, it just
disappears). But it's also the formula for infinite exchangeability, and
...
conditions (imagining to joint together their recording times). This is the
...
approximated using the Shannon entropy $\ysh$, which satisfies the bounds
\citep[Lemma 2.2 pp. 429--430]{csiszaretal2004b}
\begin{equation}
...
using the definition of Shannon entropy. \mynote{Is it possible that this
  approximation affects the result? Check this!}
...
appears in an exponential. We can therefore approximate it with a delta.

This delta restricts $\yJ$ to the form $\yG\cdot\ynu$, making it completely
determined by $\ynu$. The density~\eqref{eq:log-density_final} therefore
collapses onto the $\ynu$ space. More precisely, a subspace of $\ynu$,
since $\yJ$ must also satisfy the constraint $\sum_{a}\yJ=\yf$, which
becomes $\sum_{a}\yG\cdot\ynu = \yf$, constraining $\ynu$.
...
$a$s. This problem can be mitigated by considering a softer constraint
between $\yJ$ and $\yf$, for example considering that there are errors in
spike sorting or just considering the first $m$ moments of $\yf$. We
express the softer constraint by replacing the delta with a normal $\Nd$ whose
covariance matrix $\ysig$ is determined from the spike-sorting error. With this
approximation we reduce our problem to the density
\begin{equation}
    \ln \pf(\ynu \| \yas, \yI) =
\ln\Nd(\tsum_{a}\yG\cdot\ynu \| \yf, \ysig) + \ln q(\ynu)
%\\[\jot]{}  - T\,\ysh(\yf) - \ln Z(\yas)
%  \text{with $\tsum_{a}\yJ=\yf(\yas)$}
\end{equation}
