...
%% Last-Updated: 2020-06-03T20:10:34+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
The network is accompanied by tables of conditional distributions.

A simple non-trivial example is this network:
\begin{center}
\begin{minipage}[t]{0.75\linewidth}
%\vspace{}%
\parbox{0.3\linewidth}{\protect\includegraphics[scale=0.5]{bayesnet3s.png}}% bayesnet.svg
\hfill%\vspace{0pt}
$\ff{x \bcond a,b}
\quad\begin{gathered}
\ff{,a}\\ \ff{,,b}
...
$\ff{x,a,b} = \ff{x \bcond a,b} \times \ff{,a} \times \ff{,,b}$.
...
specific person, given the age, gender, health condition of that person,
and other factors. Sometimes their application seems to refer to a whole
\emph{population} or even superpopulation, and therefore to involve
\enquote{long-run} frequencies. Sometimes the same application seems to
shift between the two points of view.\citep[See
...
The two applications are not equivalent. In particular, the distributions
they involve are numerically different. In general we cannot apply to a
population the conditional-probability tables of a Bayesian network
constructed for an individual, nor vice versa. De~finetti's theorem shows
why: empirical frequencies (\cf\ $F_{x,a}$), superpopulation frequencies
(\cf\ $f_{x,a}$), and probabilities for individual instances (\cf\
$\p(\X{1}\=\x{1} \| \dotso)$), are numerically different. They may tend to
become approximately equal after updating on very large amounts of data.
But in practical applications, especially in medicine and social sciences,
we do \emph{not} have large amounts of data.

Several questions thus arise in applying a Bayesian network. In the
application to individuals, how are our (conditional or unconditional)
degrees of belief about the specific instance related to, or updated by,
