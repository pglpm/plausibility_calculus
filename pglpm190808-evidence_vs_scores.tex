...
%% Last-Updated: 2019-08-11T12:34:44+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
hypothesis $H_{h}$ given data $D$ and background knowledge $\yK$, that is,
$\p(H_{h} \| D \, \yK)$, is related to our \dob\ in observing those data
when we entertain that hypothesis as true, that is,
$\p(D \| H_{h} \, \yK)$:
\begin{equation}
...
hypothesis given the data \citep[\sect~6.1, p.~62]{good1950}. $D$, $H_{h}$,
$\yK$ denote propositions, which usually are about numeric quantities. I'll
use the terms \enquote{\dob}, \enquote{belief}, and \enquote{probability}
as synonyms.

The post-data belief~\eqref{eq:posterior_hypothesis} is of course
conditional on the set $\set{H_{h}}$ of mutually exclusive and exhaustive
hypotheses under consideration, which is implicit in the knowledge $\yK$ --
in fact, the above formula is only valid if
\begin{equation}
  \label{eq:implicit_knowledge}
  \p\bigl(\tlor_{h} H_{h} \| \yK\bigr) = 1,
  \qquad
  \p(H_{h} \land H_{h'} \| \yK) = 0 \quad \forall h \ne h'.
\end{equation}
