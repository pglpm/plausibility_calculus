...
%% Last-Updated: 2019-01-04T00:41:45+0100
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Report-no: ***
\newif\ifarxiv
\arxivfalse
...
\newcommand*{\propertitle}{What is probability model?}
...
A learning model is one that yields different plausibilities about some
data $(y_1, y_2, \dotsc)$ conditional on $(x_1, x_2, \dotsc)$ if we add
knowledge about other data $(y_1', y_2', \dotsc)$, $(x_'1, x_2', \dotsc)$:
\begin{equation}
  \label{eq:learning_def}
  
\end{equation}
$$\mathrm{p}(d \mid c, d', c', M) \ne \mathrm{p}(d \mid c, M).$$

A non-learning model is one for which our plausibilities about some data $d$ conditional on $c$ are not affected by knowledge about other data $d'$, $c'$:
$$\mathrm{p}(d \mid c, d', c', M) = \mathrm{p}(d \mid c, M).$$
This means that if we have several data $(d_1, d_2, \dotsc)$, $(c_1, c_2, \dotsc)$ then
$$\mathrm{p}(d_1, d_2, \dotsc \mid c_1, c_2, \dotsc, M) = \prod_i \mathrm{p}(d_i \mid c_i, M).$$

It is \emph{non-learning} models that people usually, ultimately seek. For example, a trained and ready-to-use neural net is a non-learning model. Parametric models are non-learning models; in fact, each value of the (possibly multi-dimensional) parameter defines a specific non-learning model.
