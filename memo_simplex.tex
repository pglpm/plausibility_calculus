...
%% Last-Updated: 2019-01-15T21:55:02+0100
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Report-no: ***
\newif\ifarxiv
\arxivfalse
...
\newcommand*{\propertitle}{Memos on densities and metrics on simplices}
...
\begin{abstract}\labelsep 0pt%
  \noindent Some memos on densities and metrics on simplices.
...
\section{Setup and base density}
\label{sec:base}
...
the integration domains. Denote
\begin{equation}
...
basic density (that is, volume element) we can take either
\begin{gather}
...
\end{equation}
The density induced by the Euclidean one is in this case given by
...
lower-dimensional subset is to equip the simplex with a metric. Let's
consider the flat metric. We find its expression in the coordinates $(q_1,
\dotsc, q_N)$ by symmetrically embedding the simplex in
$\clop{0,+\infty}^{N+1}$ as a flat hypersurface and pulling back the
Euclidean metric onto it. We use the embedding
\begin{equation}
  \label{eq:embedding}
  (q_1, \dotsc, q_N) \mapsto (x_0, x_1,\dotsc, x_N) =
  ( 1-q_1 -\dotsb -q_N, q_1, \dotsc, q_N),
\end{equation}
which has tangent map
\begin{gather}
...
\end{pmatrix}}_{N}
\end{gather}
and $\yid$ is the identity matrix.
...
diagonal. Its determinant can be found with Sylvester's theorem \citep{sylvester1851,akritasetal1996}:
\begin{equation}
  \label{eq:sylvester}
