...
%% Last-Updated: 2021-11-07T13:56:44+0100
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
\item\label{item:conditional_exchangeability} \emph{Conditional
    exchangeability.} The prior distribution is conditionally exchangeable
  in the predictands given the predictors, but not in the predictors across
  future and training subsequences. That is, the probability is the same
  for any sequence obtained from $\bz\bzz$ by exchanging predictand values
  between different instances in the sequence \emph{that have the same
    predictor values}; however, the probability is generally not the same
  if we exchange predictor values, especially if across future data and
  training data.
\end{enumerate}
...
however. potentially come from different populations. In other words, every
...
training data come from the same source (or, again, \enquote{population} if
you like) as the future data.
Assumption~\ref{item:conditional_exchangeability} applies to cases where
our training data come from a somewhat different source, possibly because
of constraints in the choice of predictors -- for example, the algorithm
will be applied to predictor values expensive to realize artificially, and
the training is based on predictor values cheaper to realize artificially.
...
is assumed in any case. It is indeed is a necessary assumptions for our
