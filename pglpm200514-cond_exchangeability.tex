...
%% Last-Updated: 2020-05-30T21:48:35+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
\newcommand*{\ff}{\bm{f}}
\newcommand*{\FF}{\bm{F}}
\newcommand*{\fc}{\bm{f_{\!x\bcond c}}}
\newcommand*{\fm}{\bm{f_{\!c}}}
...
Denote (with some abuse of the symbols) by $\fc \defd (f_{x \bcond c})$ the
...
\begin{equation}
  ...
  \| I) \;.
\end{equation}
Besides the set of conditional distributions $\fc$, consider also the
joint distribution $\fj$ and the marginal distributions $\fm$ over $c$
values. Both distributions are defined in an obvious way.

Assume the following:
\begin{enumerate}[wide]
\item the marginal distribution for the $\C{i}$ is fully exchangeable,
\item the distribution for the $\X{i}$ is conditionally exchangeable given
  the $\C{i}$.
\end{enumerate}

Then the joint distribution~\eqref{eq:joint_p_xc} is fully exchangeable for
the pairs $\set[\big]{(\x{i}, \cc{i})}$, and it can be written as
\begin{multline}
  \label{eq:result_freq}
\p( \X{1}\=\x{1}, \C{1}\=\cc{1},\; \dotsc,\;  \X{N}\=\x{N},\C{N}\=\cc{N}
\| I) ={}\\
\int\prod_{c,x} {f_{x,c}}^{N F_{x, c}}\;
%\pf\condt[\big]{(f_{x \bcond c})}{I} \,\di(f_{x \bcond c}) \;.
\pf\cond[\big]{\fc}{I}\,
\pf\cond[\big]{\fm}{I}\,
\,\di\fc\, \di\fm \;.
