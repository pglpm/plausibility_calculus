...
%% Last-Updated: 2021-11-08T13:46:40+0100
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
\newcommand*{\firstdraft}{8 November 2021}
\newcommand*{\firstpublished}{\firstdraft}
\newcommand*{\updated}{\ifarxiv***\else\today\fi}
\newcommand*{\propertitle}{Note on conditional exchangeability%\\{\large ***}%
}% title uses LARGE; set Large for smaller
\newcommand*{\pdftitle}{\propertitle}
\newcommand*{\headtitle}{Conditional exchangeability}
...
\section{Exchangeability of pairs}
\label{sec:exch_pairs}

Our domain of discourse consists of a countable set of atomic statements
\begin{equation}
  \label{eq:basic_statements}
  \begin{gathered}
    X_{ni},\quad n \in \NN,\ i \in \yX,
    \qquad
  \text{\small $X_{na}, X_{nb}$ mutually contradictory for $a \ne b$},
    \\
  Y_{nj},\quad n \in \NN,\ j \in \yY,  
    \qquad
  \text{\small $Y_{na}, Y_{nb}$ mutually contradictory for $a \ne b$}.
\end{gathered}
\end{equation}
Typically these statements express observations or measurement outcomes of
some quantities and have the form \enquote{$Z_{n} = z_{i}$}.

A probability distribution is called \emph{jointly exchangeable} in $X$ and
$Y$ if
\begin{multline}
  \label{eq:joint_exch}
  \P(
...
A probability distribution is called \emph{conditionally exchangeable} in $Y$ given
$Y$ if
\begin{multline}
  \label{eq:conditional_exch}
  \P(
Y_{1 j_{1}}
,  \dotsb,
Y_{n j_{n}}
...
\\\text{for all $m,n$} \ .
\end{multline}
This definition correspond to the alternative definition given in
Appendix~2 of \textcite{lindleyetal1981}.

If the probabilities for $X,Y$ are jointly exchangeable then the
conditional probabilities for $Y$ given $X$ are conditionally exchangeable.
If the conditional probabilities for $Y$ given $X$ are conditionally
exchangeable and $X$ is exchangeable, then $X,Y$ are jointly exchangeable.

If the conditional probability for $Y$ given $X$ is conditionally
exchangeable, then we can write
\begin{multline}
  \label{eq:repres_conditional}
    \P( Y_{1 j_{1}} ,  Y_{2 j_{2}} ,  Y_{3 j_{3}} ,  \dotsb \|
X_{1 i_{1}} ,  X_{2 i_{2}} ,  X_{3 i_{3}} ,  \dotsb,\ H )
={}\\
\int
\Bigl(\prod_{n} F_{j_{n}\| i_{n}}\Bigr) \
\p\Bigl[(F_{b\|a}) \|[\Big] \bigwedge_{n} X_{n i_{n}},\ H\Bigr] \ \di(F_{b\|a})
\end{multline}
Note in particular that the probability $\p\bigl[(F_{b\|a}) \|
\dotso,\ H\bigr] \ \di(F_{b\|a})$ can depend on the long-run frequencies of
$X$ in the conditional.
