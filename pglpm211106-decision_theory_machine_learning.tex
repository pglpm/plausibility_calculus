...
%% Last-Updated: 2021-11-06T21:59:00+0100
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
to the space of coefficients of a linear-regression algorithm). With some
abuse of notation we shall get rid of \enquote{$t$} and denote the output
produced by the \ml\ algorithm \amp\ parameter $\theta$ operating on the
input $x$ simply as $\theta(x)$.

We adopt this general point of view that combines algorithm and parameter
...
$\theta$ and scenario (future data) $\bz$ we must now specify the utility
$U(\theta \| \bz)$.
...
approach, where the functional form of the utility changes with each
instance (even if $\yx{n}$ and $\yy{n}$ assume the same pair of values), is
also possible and may be realistic in particular situation.

Further, in each single instance what we are actually choosing is an output
value $\theta(x)$, determined by the input $x$ and by the algorithm and
its parameter $\theta$. The single-instance unknown \enquote{scenario} is
the true value $y$. So the single-instance utility $u(\theta \|
\yx{n}\yy{n})$ can actually be rewritten as
\begin{equation}
...
\end{equation}
This equation express the fact that the choice of parameter $\theta$ is
...
hypotheses or background knowledge $H$, and on a sequence of known inputs
$(\yxx{\nu})$ and corresponding \emph{known} outputs $(\yyy{\nu})$, with
$\yxx{n} \in X$ and $\yyy{n} \in Y$. Analogously to the scenarios we denote
$\bxx \defd (\yxx{1}, \yxx{2}, \dotsc)$, analogously for $\byy$, and
$\bzz \defd (\yxx{1}\yyy{1}, \yxx{2}\yyy{2}, \dotsc)$. Our probability
...
*** cost part and inferential part may involve completely different
algorithms. Possibly suggests inconsistency in letting each competing
algorithm to do its own inference.

*** optimization crucially depends on probabilities for $\bx$. This is
...
*** $\argsup$ may be separated into optimization of parameter for each
