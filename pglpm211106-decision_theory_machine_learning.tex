...
%% Last-Updated: 2021-11-06T16:48:32+0100
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
\newcommand*{\bxx}{\widebar{\xi}}
\newcommand*{\byy}{\widebar{\upsilon}}
\newcommand*{\bzz}{\widebar{\zeta}}
...
The output of a trained \ml\ algorithm, however, is often a
specific value taken at face value; that is, treated as \enquote{the
...
at some specific value. This also represents a choice made on our part.
...
training and choosing the internal parameters of a \ml\
algorithm that performs regression or classification, that is, that outputs
a specific value $y$ (continuous or categorical) given an input $x$ (which
can be real, categorical, or belong to some general manifold). A
brief summary of decision theory is given in the next section.
...
We must choose one among several \ml\ algorithms, and for each of them, one
among several internal parameter values. Note that this nested choice can
actually be combined into a single choice. Label the candidates algorithms
with 1, 2, \etc, and denote their parameter spaces by $\varTheta_{1}$,
$\varTheta_{2}$, \etc. The choice of algorithm and internal parameter can
then be seen as the choice of a parameter value $\theta$ in the union space
$\varTheta_{1} \cup \varTheta_{2} \cup\dotsb$, denoted $\varTheta$. A \ml\
algorithm with internal parameter $\theta$ typically outputs a value that
is a function $t(x \| \theta)$ of the input $x$ and of the (fixed)
parameter $\theta$. From our general point of view it is implicitly
understood that \enquote{$t$} can actually have different functional forms
depending on whether $\theta \in \varTheta_{1}$ or
...
to the space of coefficients of a linear-regression algorithm).

For the moment we work with this general point of view, and later explore
how the separation into two different kinds of choices is made. Our
possible choices or decisions therefore consist of the possible values
$\theta \in \varTheta$.
...
consist of all possible sequences of pairs
$\bigl((\yx{1}, \yy{1}), (\yx{2}, \yy{2}), \dotsc \bigr)$ that our
algorithm will encounter in its lifetime: the $\yx{n}$ will be the known
inputs fed to the algorithm, and the $\yy{n}$ the unknown values that the
...
Denote $\bx \defd (\yx{1}, \yx{2}, \dotsc)$, analogously for $\by$, and
$\bz \defd (\bx, \by) \defd \bigl((\yx{1}, \yy{1}), (\yx{2}, \yy{2}), \dotsc \bigr)$.
If the quantity $x$ takes values in the manifold $X$ and $y$ in $Y$, 
our possible scenarios live in the space $\prod\limits_{n}(X\times Y)$.

\subsection{Utilities}
\label{sec:utilities}

For each combination of decision (algorithm \amp\ internal parameter)
$\theta$ and scenario (future data) $(\bx, \by)$ we must now specify the
utility $U(\theta \| \bz)$.

We make the realistic assumptions that this utility is the sum of utilities
$u[\theta \| (\yx{n}, \yy{n})]$ from each single application $n$ of the
algorithm to the sequence of data, and that such individual utilities have
identical functional forms:
\begin{equation}
  \label{eq:total_utility}
  U(\theta \| \bz) = \sum_{n} u(\theta \| \yx{n}, \yy{n}) \ .
\end{equation}
This assumption simplifies the calculations to follow. A more general
approach, where the utilities change with time, is also possible and may be
realistic in particular situation.

The functional form of the single-instance utility
$u(\theta \| \yx{n}, \yy{n})$ depends on the specific problem -- in fact
it is no less problem-specific than the choice of \ml\ algorithm -- so we
do not make any assumptions about it for the moment.

\subsection{Probabilities for the scenarios and exchangeability}
\label{sec:scenario_probabilities}

Lastly we need to assess the distribution of probability $\p(\bz)\,\di\bz$
over the possible scenarios $\bz$. This probability distribution is
conditional on some hypotheses or background knowledge $H$, and on a
sequence of known inputs $(\yxx{\nu})$ and corresponding \emph{known}
outputs $(\yyy{\nu})$. Analogously to the scenarios we denote
$\bxx \defd (\yxx{1}, \yxx{2}, \dotsc)$, analogously for $\byy$, and
$\bzz \defd (\bxx, \byy) \defd \bigl((\yxx{1}, \yyy{1}), (\yxx{2},
\yyy{2}), \dotsc \bigr)$. So our probability distribution is
\begin{equation}
  \label{eq:prob_distr_conditional}
  \p(\bz \| \bzz, H)\,\di\bz \equiv
  \p(\bx, \by \| \bzz, H)\,\di\bx\,\di\by \ .
\end{equation}
...
\begin{equation}
  ...
  \begin{split}
  E(\theta \| \bz, H) 
&\defd \iint U(\theta \| \bx, \by) \ \p(\bx, \by \| \bzz, H)\,\di\bx\,\di\by
\\
&=\iint \sum_{n} u(\theta \| \yx{n}, \yy{n}) \
\p(\bx, \by \| \bzz, H)\,\di\bx\,\di\by
\end{split}
\end{equation}
and we must choose the algorithm and internal parameters $\ctheta$ given by
\begin{equation}
