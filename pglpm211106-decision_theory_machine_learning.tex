...
%% Last-Updated: 2021-11-16T08:46:28+0100
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
\begin{subequations} \label{eq:exchangeable_conditional}
  \begin{gather}
    ...
    \p(\by \| \bx, \bzz, H) =
    \int \Bigl[\prod_{n} F(\yy{n}\|\yx{n})\Bigr] \ \p(F \| \bzz, H)\ \di F
    \\\shortintertext{with}
...
\begin{subequations}    \label{eq:exp_utility_replace}
  \begin{gather}
    ...
    \!\begin{multlined}[t][0.7\linewidth]
      \frac{1}{Z(\bzz)}\iiiint \sum_{n} u[\yy{n} \| \theta(\yx{n})]\times{}\\ 
      \Bigl[\prod_{m} F(\yy{m}\|\yx{m})\ G(\yx{m})\Bigr]\
      \Bigl[\prod_{\nu} F(\yyy{\nu}\|\yxx{\nu})\Bigr]\times{}\\
      \p(F \| H)\
      \p(G \| \bzz, H)\ 
      \di\bz\,\di F\,\di G
  ...
  \end{gather}
\end{subequations}
This expression can be simplified exchanging the sum in $n$ and the
integrals, and then integrating over $\yy{m}\yx{m}$ for which $m \ne n$;
such integrals give unity since $G$ and each $F$ are normalized. We obtain
\begin{equation}
  ...
  \begin{split}
    ...
    \!\begin{multlined}[t][0.65\linewidth]
      \frac{1}{Z(\bzz)}\sum_{n} \iiiint u[\yy{n} \| \theta(\yx{n})] \
      F(\yx{n}\|\yy{n})\ G(\yx{n}) \times{}\\
      \Bigl[\prod_{\nu} F(\yyy{\nu}\|\yxx{\nu})\Bigr]\ 
      \p(F \| H)\ \p(G \| \bzz, H)\
      \di\yy{n}\,\di\yx{n}\,\di F\,\di G
\end{multlined}
\\[3\jot]&\propto
    \!\begin{multlined}[t][0.65\linewidth]
      \iiint u[y \| \theta(x)] \
...
$\yx{n}\yy{n}$ to $xy$, and performed a formal integration over $\di G$.
The terms of the sum in $n$ are therefore all equal, and the utility is a
simple multiple of any such term. We also omit the $\theta$-independent
factor $Z(\bzz)$ and simplify the integral notation. Thus the final
expected utility of $\theta$, besides a constant factor, is
\begin{multline}
...
\begin{empheq}[box=\widefbox]{multline}
  ...
  \ctheta \defd \argsup_{\theta} \int u[y \| \theta(x)] \
      F(y\|x)\ 
      \Bigl[\prod_{\nu} F(\yyy{\nu}\|\yxx{\nu})\Bigr]\ 
      \p(F \| H)\times{}\\
      \p(x \| \bzz, H)\
