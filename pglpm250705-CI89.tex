...
%% Last-Updated: 2025-09-06T23:42:52+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
\newcommand*{\headtitle}{89\% credibility intervals}
...
\begin{abstract}\labelsep 0pt%
  \noindent A curious property of 89\% credibility intervals is discussed.
...
A Bayesian \emph{credibility interval} (also called \enquote{credible} or \enquote{compatibility} interval) is a range where the true value of some real quantity lies, with a given probability. For instance, if $\clcl{1.2, 7.0}$ is a 60\% credibility interval for the quantity $Q$, then there is a 60\% probability that the true value of $Q$ is between $1.2$ and $7.4$ (note the difference in meaning from a \emph{confidence} interval, whose definition is much more involved and less direct).

It is of course always best to report the full posterior probability distribution of a quantity of interest, so that the reader can derive from it the probabilities of any desired range as well as other properties. But a credibility interval often provides a useful summary of the full probability distribution.

Credibility intervals of 50\%, 75\%, 90\%, 95\%, or even 99\% probability are more or less commonly reported in the Bayesian literature. Also \emph{89\%} credibility intervals have become quite fashionable since around 2016. They seem to originate in McElreath's \emph{Statistical Rethinking} \parencites*{mcelreath2016_r2020}, first published in that year. McElreath rightly points out\autocites[\eg \sect~3.2.2 p.~56, \sect~4.3.5 p.~88]{mcelreath2016_r2020}
 that the specific probability value should be chosen depending on the context; otherwise it's just arbitrary. To emphasize and remind us of this fact, he uses 89\%, which is an arbitrary value.

Or so he thought. It turns out that 89\% credibility intervals do possess a peculiar property.

With a $p$ credibility interval, the true value lies within the interval with probability $p$, and without with probability $1-p$. We can quantify our uncertainty about the two possibilities by means of the Shannon entropy \autocites[Items 13-24, 13-25]{iso2008d}:
\begin{equation}
  \label{eq:shannon_def}
  H(p) \defd - p \log_{2} p - (1-p) \log_{2}(1-p)\:\unit{Sh} \,.
\end{equation}
The value of \qty{1}{Sh} represent an equal uncertainty between two alternatives.

Thus for a 50\% credibility interval we have an uncertainty of \qty{1}{Sh} on whether the true values lies within or without. This is the highest lever of uncertainty we can have. It is useful to also provide an interval where we are somewhat more certain that the true values lies within than without.

We can ask: for which interval is our maximal uncertainty halved: \qty{0.5}{Sh}? The answer is: \emph{an $89.00\%$ credibility interval}. This result is easily calculated numerically from \eqn~\eqref{eq:shannon_def}.

A more precise value is $88.9972\%$; the entropy for the exact probability $89\%$ is approximately \qty{0.499916}{Sh}.

When we report a 50\% and an 89\% credibility interval we are therefore reporting intervals with uncertainties of one shannon and of half a shannon.

Of course the arbitrariness remains. We could also report intervals with uncertainties \qty{1}{Sh}, $\frac23\:\unit{Sh}$, $\frac13\:\unit{Sh}$; their credibilities would be $50\%$, $82.6\%$, $93.9\%$. Or with uncertainties \qty{1}{Sh}, \qty{0.5}{Sh}, \qty{0.25}{Sh}: $50\%$, $89.0\%$, $95.8\%$.
