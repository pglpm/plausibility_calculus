...
%% Last-Updated: 2019-10-31T13:53:44+0100
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
distribution $(x_{i})$ satisfies the bounds \footfullcite[Lemma 2.2 pp.
...
\label{sec:approx_final}

We consider two successive approximations of the
log-density~\eqref{eq:log-density_final}, based on the fact that $T$ is
large.

\bigskip

The relative entropy $\ysh(\yJ; \yG\cdot\ynu)$ is always positive, and
vanishes if only if $J = \yG\cdot\ynu$. This term is multiplied by $-T$ and
appears in an exponential. We therefore approximate it with a delta.

This delta restricts $\yJ$ to the form $\yG\cdot\ynu$, making it completely
determined by $\ynu$. The density~\eqref{eq:log-density_final} thus
collapses onto the $\ynu$-space. More precisely a subspace, since $\yJ$
must also satisfy the constraint $\sum_{a}\yJ=\yf$, which becomes
$\sum_{a}\yG\cdot\ynu = \yf$, constraining $\ynu$.

Such a constraint is actually too strong, in the sense that no distribution
$\ynu$ can satisfy it -- it leads to negative values $nu_{a}$ for some
$a$s. This problem is mitigated by considering a softer constraint between
$\yJ$ and $\yf$, for example taking into account errors in spike sorting or
just considering the first $m$ moments of $\yf$. We express the softer
constraint by replacing the delta with a normal $\Nd$ whose covariance
matrix $\ysig$ is determined from the spike-sorting error. With this
approximation we reduce our problem to the density
\begin{empheq}[box=\widefbox]{equation}\label{eq:maxent_bayes_approx}
    \ln \pf(\ynu \| \yas, \yI) \propto
    \ln\Nd(\tsum_{a}\yG\cdot\ynu \| \yf, \ysig) 
   - L\;\ysh(\ynu;\yr),
%\\[\jot]{}  - T\,\ysh(\yf) - \ln Z(\yas)
%  \text{with $\tsum_{a}\yJ=\yf(\yas)$}
\end{empheq}
where $q(\ynu)$ has been replaced by the entropic
density~\eqref{eq:density_nu_entropic}. We can call this the
\enquote{maximum-entropy approximation with Bayesian correction}. It can
also be represented by a set of Monte Carlo samples $\set{\ynu^{(i)}}$.

\bigskip

As a further approximation we can simply consider the mode
of~\eqref{eq:maxent_bayes_approx}. This is found by maximizing the relative
entropy in its expression under the moment constraints about $\yf$. The solution
is the one given by the maximum-entropy method applied to the full network
with subnetwork constraints.

\bigskip

Figure A shows comparison of the expected values (means of the Monte Carlo
samples) for the density~\eqref{eq:log-density_final} (for $\yF$), the
density~\eqref{eq:maxent_bayes_approx} (for $\ynu$ and using the first six
moments of $\yf$ as constraints), and the maximum-entropy solution (six
moments as constraints).
\begin{figure}[!b]
\centering
\includegraphics[width=\linewidth]{maxent_latest.png}%
\caption{Comparison of~\eqref{eq:log-density_final},
  \eqref{eq:maxent_bayes_approx}, and the maximum-entropy approximation
  (the vertical-axis label is wrong).}
\label{fig:compare_approxs}
\end{figure}% maxent_solutions_jointfreq.nb
