...
%% Last-Updated: 2025-09-07T10:57:52+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
A Bayesian \emph{credibility interval} (or \enquote{credible} or \enquote{compatibility} interval) is a range where the true value of some real quantity lies with a given probability. For instance, if $\clcl{1.2, 7.0}$ is a 60\% credibility interval for the quantity $Q$, then there is a 60\% probability that the true value of $Q$ is between $1.2$ and $7.0$ (note the difference in meaning from a \emph{confidence} interval, whose definition is much more involved and less direct).

It is always best to report the full posterior probability distribution of a quantity of interest, so that the reader can calculate from it the probabilities of any desired ranges as well as other properties. But credibility intervals often provide useful summaries of the full probability distribution.

Credibility intervals of 50\%, 75\%, 90\%, 95\%, or even 99\% probability are more or less commonly used in the Bayesian literature. Also \emph{89\%} credibility intervals have become quite fashionable since around 2016. They seemingly originate in McElreath's \emph{Statistical Rethinking} \parencites*{mcelreath2016_r2020}, first published in that year. McElreath justly points out\autocites[\eg \sect~3.2.2 p.~56, \sect~4.3.5 p.~88]{mcelreath2016_r2020} that the specific probability value of a credibility interval should be chosen depending on the context; there is nothing special about values like 90\% or 95\%. To emphasize and remind us of this fact, he uses 89\%, which is an arbitrary value.

Or so he thought. It turns out that 89\% credibility intervals do possess a peculiar property.

With a $p$ credibility interval, the true value lies inside the interval with probability $p$, and outside with probability $1-p$. We can quantify our uncertainty about the two possibilities by means of the Shannon entropy: \autocites[Items 13-24, 13-25]{iso2008d}
\begin{equation}
...
Thus for a 50\% credibility interval we have an uncertainty of \qty{1}{Sh} on whether the true value lies inside or outside. This is the highest level of uncertainty for this dichotomous alternative. It is useful to also provide an interval with an uncertainty smaller than \qty{1}{Sh}, so that we are somewhat more certain that the true value lies inside than outside.

For example, with which interval is our maximal uncertainty \emph{halved} to \qty{0.5}{Sh}? The answer is: with \emph{an $89.00\%$ credibility interval}. This result is easily calculated solving $H(p) = \qty{0.5}{Sh}$ numerically. A more precise solution is $88.9972\%$; vice versa the entropy corresponding to the exact probability $89\%$ is approximately \qty{0.499916}{Sh}.
