...
%% Last-Updated: 2018-12-24T19:09:35+0100
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Report-no: ***
\newif\ifarxiv
\arxivfalse
...
the second quantity is given by a function of the first. This is the case
in many classification problems and similar inferences. The focus in this
kind of problems is to infer $x$ in an observation with a new value of $t$,
given observations of the pair of quantities for different values of $t$.

If we assume for the moment that $t$ and $x$ assume finite sets of values,
then also the set of functions between them is finite. Regression in the
restricted sense above is then equivalent to inferring a function
$f\colon t \mapsto x$:
\begin{equation}
  \label{eq:x_from_t_marginal_f}
  \pf(x \|t, \yI) = \int\di f\; \pf(x \| t, f, \yI)\;\pf(f \| \yI),
\end{equation}
with
\begin{equation}
  \label{eq:x_from_t_given_f_deterministic}
  \pf(x \| t, f, \yI) = \delt[x - f(t)].
\end{equation}
