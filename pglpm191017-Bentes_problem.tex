...
%% Last-Updated: 2019-11-03T15:06:01+0100
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
\newcommand*{\propertitle}{Notes on inferring connectivity\\(Bente's
  problem) [draft]%\\{\large ***}%
...
We're considering neurons in a specific region $A$ of the mouse brain, and
neurons in a specific region $B$ which connects to those in region $A$. Our
question is how many neurons in region $B$ project, on average, to a single
neuron in region $A$.

First of all let's make our question more precise. We can rephrase it in
...
reached by the virus. A data, we have the number of days between virus
...
For each mouse consider all neurons in the input region, label them with
$i=1,2,\dotsc$; and all neurons in the target region, label them
$j=1,2,\dotsc$. Mouse $m$ has $K_{m}$ neurons in the input region $B$ and
$L_{m}$ neurons in the target region $A$.

Now that we've labelled the mice and their neurons in the two regions, we
can make several concrete statements, such as
\[\parbox{0.95\linewidth}{\enquote{In mouse \#\,64, the neuron \#\,931 in the input
...
\begin{equation}
  ...
  \yC_{mij} \defd \parbox[t]{0.8\linewidth}{\enquote{In mouse $m$, neuron $i$ in input region projects to neuron $j$ in target region}}
\end{equation}
...
questions~\ref{item:Q1} and \ref{item:Q2} are built up from the specific
statements~\eqref{eq:statement_connection}, counting how many of them are
true and how many are false. Therefore the probabilities of our answers
depend on the probabilities of the individual
statements~\eqref{eq:statement_connection}.
