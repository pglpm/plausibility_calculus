...
%% Created: 2018-12-19T16:51:04+0100
%% Last-Updated: 2018-12-20T01:24:13+0100
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Report-no: ***
\newif\ifarxiv
\arxivfalse
...
\newcommand*{\firstdraft}{19 December 2018}
\newcommand*{\firstpublished}{***}
\newcommand*{\propertitle}{Ising models and sufficiency in neuroscience}
\newcommand*{\pdftitle}{\propertitle}
\newcommand*{\headtitle}{Ising models \amp\ sufficiency}
...
Consider a large population of neurons. Imagine that its activity has been
time-binned and binarized. Consider a specific time bin or one chosen with
an unknown method.

We want to infer something about the activity of this population at this
time bin. We want our inference to be informed with the activity observed
in a small sample of the population.

The first question is what's the \enquote{something} we want to infer. It's
difficult to proceed without a clear idea about it. I assume we are
interested in knowing the full activity. Denote it by $\yS \defd (\ySS_i)$,
with $i$ indexing the neurons.


Denote the activity observed in the sample by $\ys \defd (\yss_j)$. We are
uncertain about $\yS$, so what we want is to quantify
\begin{equation}
  \label{eq:p_Ss}
  \pf(\yS \| \ys, \yI)
\end{equation}
where $\yI$ is our knowledge about the full population besides what's
observed in its sample.

By Bayes's theorem,
\begin{equation}
  \label{eq:p_Ss_bayes}
  \pf(\yS \| \ys, \yI) \propto \pf(\ys \| \yS, \yI)\;\pf(\yS \|\yI),
\end{equation}
so we need to specify our initial uncertainty about $\yS$ and our sampling
uncertainty $\pf(\ys \| \yS, \yI)$.
