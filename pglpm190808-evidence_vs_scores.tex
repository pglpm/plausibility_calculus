...
%% Last-Updated: 2019-08-08T10:10:30+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
\begin{equation}
  ...
  \p(H_{h} \| D \yK) = \frac{\p(D \| H_{h} \yK)\;\p(H \| \yK)}{\p(D \| \yK)}.
\end{equation}
The probability in the denominator is usually calculated by considering a
set of mutually exclusive and exhaustive hypotheses $\set{H_{h}}$
comprising $H_{h}$ and using the law of total probability:
\begin{equation}
  \label{eq:data_total_prob}
  \p(D \| \yK) = \sum_{h} \p(D \| H_{h} \yK)\; \p(H_{h} \| \yK).
\end{equation}
Let's call $\p(H_{h} \| D \yK)$ the post-data probability or
belief in $H_{h}$. If our pre-data beliefs $\p(H_{h} \| \yK)$ in the hypotheses
under consideration are all equal, then the ratio of the post-data
probabilities for the various hypotheses is equal to the ratio of the
beliefs in the data given the hypotheses:
\begin{equation}
  \label{eq:ratios_posteriors}
  \frac{\p(H_{h} \| D \yK)}{\p(H_{h'} \| D \yK)} =
  \frac{\p(D \| H_{h} \yK)}{\p(D \| H_{h'} D \yK)}
  \quad\text{\footnotesize (if $\p(H_{h} \| \yK) = \p(H_{h'} \| \yK)$)}.
\end{equation}
...
some time, various other measures to compare our beliefs in a set of hypotheses
given some data.
