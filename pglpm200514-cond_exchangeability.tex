...
%% Last-Updated: 2020-05-31T19:03:52+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
of $\NN$, to avoid a proliferation of subscripts; it should be read as
\enquote{$\set{i_{1}, i_{2}, \dotsc, i_{N}}$}.

\medskip

Denote by $\ff \defd (f_{x})$ a normalized distribution over the values
...
\equiv  \int\prod_{x} {f_{x}}^{N F_{x}}\;   \pf(\ff \| I)\,\di\ff \;,
\end{multline}
where the integral is over the simplex of distributions $\set{\ff}$.
...
distribution of probability to the statements $\set[\big]{\X{i}\=\x{i}}$,
if you knew the true $\set[\big]{\C{i}\=\cc{i}}$. But you do not know the
...
Let us suppose that we our uncertainty about the $\set{\C{i}}$ is expressed
by a fully exchangeable marginal probability distribution.
...
The density
$\pf\cond[\big]{\fc}{I}\, \pf\cond[\big]{\fm}{I}\, \,\di\fc\, \di\fm$ is
just a special case of a density over the joint frequencies,
$\pf\cond[\big]{\fj}{I}\, \,\di\fj$, since the variables $(\fc,\fm)$ are in
one-one correspondence with $\fj$.
