...
%% Last-Updated: 2019-11-06T14:27:19+0100
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
\section{Probabilistic formulation of the problem}
...
those that we might observe in the future. Let's label each mouse with an
index $m=1,2,\dotsc$. For our inferences to be valid, these mice must be
judged \enquote{similar} in some respects. For each mouse, we consider all
neurons in its input region, labelling them with the index
$i=1,2,\dotsc, K_{m}$, and all neurons in its target region, labelling them
with the index $j=1,2,\dotsc, L_{m}$. These indices are arbitrary, but $m$
may be chosen to reflect a temporal order and $i$, $j$ each to reflect a
topographical order. The number $K_{m}$ of neurons in the input region and
the number $L_{m}$ in the target region will be different for each mouse
$m$.
...
With this histogram the question \enquote{how many inputs from the input
  region does a neuron in the target region have on average?} has a
definite meaning: it is the mean of the histogram,
\begin{equation}
