...
%% Last-Updated: 2019-11-06T11:42:21+0100
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
We're considering neurons in some specific region of the mouse brain, and
neurons some other specific region, which connect to those in the first
region. Let's call the first \emph{target region}, and the second
\emph{input region}. Our question is, roughly: how many neurons in the
input region project, on average, to a single neuron in the target region?

Our question actually concerns several input regions at once. For the
moment we focus on setting up the calculation for one input region only,
but the calculation will generalized to several input regions later.
Considering several at once will actually improve the results, even if just
a little.

\medskip

\textbf{The observations and data we use}: in several mice a small number
of neurons in region $A$ -- \emph{starter neurons} -- are infected with a
virus. This virus spreads, with time, to neurons -- \emph{input neurons} --
that projects to the starter neurons. The virus can't spread further than
that. Some of the infected neurons will be in region $B$. The virus also
leads to colouring of starter and input neurons. Each mouse is killed after
several days (different for each mouse) and the starter and input neurons
are counted. So our data are the number of starter and input neurons for
several mice.

There's a complication: if the mouse is killed too early, the virus hasn't
had the time to spread to all input neurons. If the mouse is killed too
late, the starter cells decay and are no longer visible. So we also need to
guess, for each observed mouse, whether there were additional starter
neurons, no longer visible, and what proportion of input neurons have been
reached by the virus. As data we have the number of days between virus
injection and killing, for each mouse. This complication for the moment is
set aside to simplify the initial calculation, but will be taken into
account later.

\section{Strategy for solving the problem}
\label{sec:strategies}

First of all let's sketch a schematic picture of our problem and make
our question more precise.

We consider a very large number of mice, those we have observed and all those
that we might observe in the future. Let's label each mouse with an index
$m=1,2,\dotsc$. For our inferences to be valid, these mice must be judged
\enquote{similar} in some respects. For each mouse we consider all neurons
in its input region, labelling them with the index $i=1,2,\dotsc$, and all
neurons in its target region, labelling them with the index $j=1,2,\dotsc$.
Mouse $m$ has $K_{m}$ neurons in the input region and $L_{m}$ neurons in
the target region. These indices are arbitrary, but $m$ may be chosen to
reflect a temporal order and $i$, $j$ each to reflect a topographical
order.

Now that we've labelled the mice and their neurons in the two regions we
can consider specific statements about their connections, such as
...
which can be either true or false. This is illustrated in
\fig~\ref{fig:cmij}: we have a grid for each mouse; each row $i$ in the
grid represents a neuron in the input region, each column $j$ a neuron in
the target region. If in mouse $m$ neuron $i$ projects to neuron $j$, then
we colour the corresponding entry $i,j$ in grid $m$ in black, or we can
associate the number 1 with it; if it doesn't project, then white or the
number 0.
%%\setlength{\intextsep}{0.5ex}% with wrapfigure
\begin{figure}[b!]%{r}{0.4\linewidth} % with wrapfigure
 \centering\includegraphics[width=\linewidth]{bente_notes_drawings.png}\\
\caption{caption}\label{fig:cmij}
\end{figure}% bente_notes_drawings.svg

Mathematically we represent these statements with these quantities:
\begin{equation}
...
whether they're true or false. But our question can be made more precise in
terms of them.

In our question we imagine to check every neuron $i$ in the target region
of every mouse $m$, counting for each how many neurons from the input
region project to it. From the point of view of \fig~\ref{fig:cmij} we're
considering all columns from all grids and counting how many black entries
each column has. If we were able to perform these observations, at the end
we could build a histogram showing the proportion of neurons in the target
regions that had $0$ inputs, $1$ input, $2$ inputs, and so on, from the
input region.

With such a histogram we the question \enquote{how many inputs from the
  input region does a neuron in the target region have on average?} has a
definite meaning: its the mean of the histogram. The histogram would tell
us even more, for example the percentage of target neurons that have
between $100$ and $200$ inputs.


Although we'll never be able to construct this histogram, we can
nevertheless guess, from our observations in a small set of mice, what kind
of shape and numerical values it could have. Our goal is to determine the
probabilities
\begin{equation}
  \label{eq:prob_histogram_general}
  \pf(\text{\small histogram} \| \text{\small data}, \yI)
\end{equation}
for all possible histograms, given the data collected in our observations
and any other information $\yI$ we have, such as our present knowledge of
brain biology. These probabilities can be found using the rule for
...
region, for each mouse. All other probabilities are calculated from this
initial probability distribution.

In the next section we discuss several possible assumptions and
approximations that we can use to build the distribution
$\pf(\text{\small histogram} \| \yI)$, making our choice among them along
the way.
