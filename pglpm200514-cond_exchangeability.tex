...
%% Last-Updated: 2020-06-05T11:32:44+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
\newcommand*{\propertitle}{Combining conditional exchangeability\\and Bayesian networks%\\{\large ***}%
}% title uses LARGE; set Large for smaller
\newcommand*{\pdftitle}{Combining conditional exchangeability and Bayesian networks}
\newcommand*{\headtitle}{Exchangeability and Bayesian networks}
...
De~finetti's theorem shows why: empirical frequencies, %(\cf\ $F_{x,y}$),
superpopulation frequencies, % (\cf\ $f_{x,y}$),
...
\begin{multlined}%[0.5\linewidth]
  \parbox{0.8\linewidth}{\small for every $N$, every subset of $N$ indices $\set{i}
    \subset \NN$, every permutation $\pi$ thereof such that\;
    $\pi(j)=k \mathrel{\;\Rightarrow\;} \cc{j}=\cc{k}$,\; and every sets of
...
Some important remarks and features of this definition will be made
shortly. For the moment
...
%     \subset \NN$, and every permutation $\pi$ thereof such that\quad
%     $\pi(j)=k \mathrel{\;\Rightarrow\;} \z{j}\eq\z{k}$,}
%   \\
%   \p\Bigl( \Land_{i}\Y{i}\=\y{i}  \|[\Big]
%   \Land_{i}(\,,\,\Z{i}\=\z{i}) \,,\,    I\Bigr) =
%   \p\Bigl( \Land_{i}\X{i}\=\x{\pi(i)}  \|[\Big]
%   \Land_{i}(\Y{i}\=\y{i}\,,\,\Z{i}\=\z{i}) \,,\, I\Bigr)
%   \;,
% \end{multline}
...
%    \X{N}\=\x{N}, \A{N}\=\va{N}, \B{N}\=\vb{N} \| I) 
   ...
   %\shoveleft{
     \int\!\prod_{x,y,z}
     (\ff{x \bcond y,z}\, \ff{,y}\, \ff{,,z})^{N \FF{x,y,z}}\;%\times{}%}\\[-\jot]
...
\begin{framed}
  ...
  \|[\Big] I\Bigr)
    ...
    % \shoveleft{
    \int\!\prod_{x,y,\dotsc}
    (\xi_{x,y,\dotsc})^{N \FF{x,y,\dotsc}}\;%\times{}%}\\[-\jot]
    \pf(\bm{\xi}\|I)\,\di\bm{\xi} 
...
$\xA$aks, and $Z$ on $\xB$ebras. If we write
...
values are \enquote{$\xA$} and \enquote{$\xB$}. We rewrite the probability
