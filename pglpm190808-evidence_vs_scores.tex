...
%% Last-Updated: 2019-08-12T23:48:52+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
\begin{equation}
  ...
  \sum_{k=1}^{d}
  \binom{d}{k}^{-1}
  \!\sum_{\mathclap{\substack{\text{ordered}\\\text{$k$-tuples}}}} \quad
  \frac{1}{k}
  \quad\sum_{\mathclap{\substack{\text{cyclic}\\\text{permutations}}}} 
...
% all hypotheses.
That is, \emph{the log-likelihood is the sum of all averaged log-scores that can be
  formed from all (non-empty) data subsets}, the averages being over 
the $\binom{d}{k}$ subsets having the same cardinality $k$.
...
\begin{equation}
  ...
  \sum_{k=0}^{d-1}
  \binom{d-1}{k}^{-1}
  \sum_{\mathclap{\substack{\text{ordered}\\\text{$k$-tuples,}\\\text{$i$ excluded}}}}
  \log\p(D_{i} \| D_{i_{1}} \dotsm D_{i_{k}} \, H \, \yK).
\end{equation}
Here we are considering each datum $D_{i}$ in turn and calculating our
log-belief in it conditional on all possible subsets of remaining data, from
the empty subset with no data (term $k=0$), to the only subset $D_{-i}$
with all data except $D_{i}$ (term $k=d-1$). These log-beliefs are averaged over
the $\binom{d-1}{k}$ subsets having the same cardinality $k$.
