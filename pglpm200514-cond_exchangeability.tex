...
%% Last-Updated: 2020-05-30T08:51:20+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
\emph{partially exchangeable}, that is, the set $\set{\X{i}}$ can be
divided into two or more categories represented by subsets
...
\end{multline}
with distinct distributions $\bg$, $\bh$ for each subset. If the density
$\pf(\bg,\bh \| \zI)$ is diagonal, that is, if it contains a term
...
category $Y$ in instances $1',2',\dotsc$, and to category $Z$ in instances
$1'',2'',\dotsc$, then
...
Let us make this information about $C$ explicit. The values of this
quantity can be \enquote{$\xY$} and \enquote{$\xZ$}. We rewrite the
probability in \eqn~\eqref{eq:partialdefinetti} as
\begin{equation}
...
exchangeability for $\set{\X{i}}$ given $\set{\C{i}}$ is expressed by
\emph{the factorizability of the density $\pf(\ff\|I)$} into the product,
\eqn~\eqref{eq:factorizable}, of a density for the conditional frequencies
$(f_{x \bcond c})$ and a density for the marginal frequencies $(f_{,c})$.
...
We have a countably infinite set of statements $\X{t}\=\x{t}$ with $t \in \set{1,2,\dots}$, where each $\X{t}$ is some quantity and each
$\x{t} \in \sX$, a common finite set of values (the theorem also works with statements that are not about variates). De~Finetti's theorem states that
