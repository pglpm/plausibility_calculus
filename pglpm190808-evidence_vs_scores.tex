...
%% Last-Updated: 2019-08-12T12:37:05+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
experiment.
\begin{innote}
  Unfortunately we too often abuse of the fact that the probability
  calculus, just like the truth calculus, proceeds purely syntactically
  rather than semantically. That is, if I tell you that $H$ and
  $H \limplies D$ are true, you can conclude that $D$ is true; similarly,
  if I tell you that $\p(H \| \yK) = p$ and $\p(H \limplies D \| \yK) = q$,
  you can conclude (try it as an exercise) that
  $\p(H\;D \| \yK) = p + q -1$. In either case you don't need to know what
  $H$ and $D$ are about -- they could very well be about Donald Duck or
  parallel universes. And so we often say \enquote{under model $\theta$ our
    belief about the value of quantity $x$ is expressed by such and such
    distribution $\pf(x \| \theta)=f(x)$}, without explaining what $\theta$
  really is and why it leads to $f$. The terms \enquote{model} and
  \enquote{hypothesis}, as often used in probability and statistics, are
  convenient and respectable-looking rugs under which we can sweep the fact
  that we don't really know what we're speaking about. The need to look
  under the rug arises, though, the moment we have to specify our pre-data
  belief, the prior, about the mysterious $\theta$.
  ...
  matters are, even more unfortunately, rarely discussed in probability and
  statistics.
\end{innote}
...
encompass and reflect the extremely complex and fuzzy hypotheses that lie
...
Expression~\eqref{eq:posterior_hypothesis_universal} is universally
valid instead, but its denominator $\p(D \| \yK)$ is effectively impossible
to quantify unless we simplify our inferential problem by introducing a
possibly unrealistically exhaustive set of hypotheses, thus falling back
to~\eqref{eq:posterior_hypothesis_sethypotheses}. We can bypass this
problem if we are content with only comparing our beliefs about any two
...
remarks about such binary comparisons, and also Good's \citey[\sect~6.3--6.6]{good1950}.


\bigskip

In a problem of \emph{selection} of hypotheses


The term $\p(D \| H_{h} \, \yK)$ is called the \emph{likelihood} of the
hypothesis given the data \citep[\sect~6.1, p.~62]{good1950}. 

The post-data belief~\eqref{eq:posterior_hypothesis} is often

We can combine our post-data beliefs in two hypotheses in different ways to
form a quantitative idea of how much the data is giving evidence for one or
the other. For example their ratio or the logarithm of their ratio, often
called \emph{weight of evidence} and \emph{Bayes factor}
\citep[\chap~6]{good1950}[\sect~1.4]{osteyeeetal1974}{mackay1992,kassetal1995}[see
...
\citep[\sect~1.4.2 p.~12]{osteyeeetal1974}. I'm here speaking of the
\emph{comparison} of hypotheses, not of their \emph{selection} -- that is,
of choosing one based on the data and discarding the others for future
calculations. For such selection, probabilities aren't enough: we also need
to specify a utility or cost function to calculate the expected gains of
choosing one or another hypothesis.
% \begin{equation}
...
other ad-hoc measures to assess our beliefs in a set of hypotheses given
some data. Here I consider one in particular, the \emph{leave-one-out
  cross-validation log-score}
\citep{krnjajicetal2011,krnjajicetal2014,geisseretal1979,vehtarietal2002,vehtarietal2012,piironenetal2017}:
\begin{equation}
  \label{eq:log-score}
...
which is the arithmetic mean on a log scale. Krnjaji\'c \amp\ Draper
\citey{krnjajicetal2011,krnjajicetal2014} compare this log-score with the
deviance information criterion  (There's an ambiguity in this
