...
%% Last-Updated: 2020-05-21T17:13:51+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
%in order to underline the general message that
\enquote{coherent comparison of a finite or countable set of alternative
  models depends on the specification (at least implicitly) of a decision
  structure, including a utility function.} \citep[\sect~6.1.3
p.~388]{bernardoetal1994_r2000}
...
In the previous section the notion of \emph{context} was mentioned but not
explained, although I trust its meaning was intuitive. By \enquote{context}
I mean the background knowledge, assumptions, and further hypotheses --
often difficult to put into words -- that are kept fixed during a
probabilistic reasoning. It is analogous to the set of axioms in a
deductive inference problem. We shall see shortly why I emphasize this
notion.

\medskip

Hypotheses are important for the inferences and forecasts they allow us to
make. We must therefore specify the full set of statements a hypothesis
refers to. Here we consider statements about observations or measurements,
of the form $\prop{X \eq x}$, typical in the sciences. And we consider the
case where the full set of statements is exhausted by the mutually
exclusive combinations (conjunctions)
\begin{equation}
  \label{eq:statements_interested}
  \propf{X_{1}\eq x_{1}} \land
  \propf{X_{2}\eq x_{2}} \land \dotsb
  \propf{X_{N}\eq x_{N}}
\end{equation}
where each value $x_{d}$ belongs to a common finite set. We may let $N$ be
arbitrarily large.
% We must consider all relevant observations, with unknown as well as known
% outcomes.

A hypothesis $H$ is well-defined in a context $\yK$ only if both together
determine the probability-values of all statements above,
\begin{equation}
...
\end{equation}
for all values of each $x_{d}$.

The emphasis above on the full set of observations and on its predictive
distribution, rather than on abstract parameters, is shared by many authors
\citep[\eg][]{definetti1930,definetti1937,roberts1965,lauritzen1974,diaconis1988,bernardoetal1994_r2000,fortinietal2000,fortinietal2012}[see
also][]{mccullagh2002}. The rest of this section shows the importance of
this point of view.
...
larger context. The hypothesis \prop{there are 7 blue and 3 red balls in the
  urn}, for example, does not determine a unique \dob\ about the draws from
the urn.

The last remark is strikingly \citep[\cf][\sect~6.1.4
p.~391]{bernardoetal1994_r2000} true for so-called \enquote{composite
  hypotheses}, having the form $\yHc \defd H_{1} \lor \dotsb \lor H_{m}$.
An example is the hypothesis \prop{the long-run relative frequency of heads
  is less than 0.4}. The probabilities of the data conditional on a
composite hypothesis depend much more strongly on the context than the
probabilities conditional on the individual hypotheses do. This happens
because, for any data $D$,
\begin{multline}
  \label{eq:comp_hyp}
  \p( D \| %H_{1} \lor \dotsb \lor H_{1'}
  \yHc \land \yK)
  ={}\\
 \p( D \| H_{1} \land \yK) \times \p(H_{1} \| \yK) +
  \dotsb +
 \p( D \| H_{m} \land \yK) \times \p(H_{m} \| \yK) \;.
\end{multline}
Thus $\p( D \| \yHc \land \yK)$ depends through $\p(H_{h} \| \yK)$ on
elements of the context that are irrelevant for the individual
$\p( D \| H_{1} \land \yK),\dotsc, \p( D \| H_{m} \land \yK)$.

In other words, a slight change in the context may not alter the
probabilities $\p( D \| H_{1} \land \yK),\dotsc$, but may drastically alter
$\p( D \| \yHc \land \yK)$. In such cases I wonder whether it is at all
meaningful to consider composite hypotheses. I have the impression that
some authors wish to compare hypotheses -- only vaguely defined in their
minds -- but do not realize that some of those hypotheses enter their
mathematics in the form of a \emph{context} instead. Their maths is not
comparing what they think. Then they find the result paradoxical. Lindley's
paradox \citep{lindley1957} seems to be an example of such
misunderstanding. ***add example from Shafer-changes context and
Hill-compares other hypotheses

\medskip

The preceding remarks are a little pedantic perhaps, but they help
understanding the argument that follows.

The emphasis on the full set of relevant observations -- with known
outcomes (data) as well as with yet unknown outcomes -- brings to light an
important difference between two kinds of hypotheses:
\begin{itemize}
\item hypotheses that \emph{do not allow learning},
\item hypotheses that \emph{allow learning}.
\end{itemize}
The
first kind satisfies the property
\begin{multline}
...
\text{$H$ learning} \liff  \p( D \| D' \land H \land \yK) \ne 
  \p( D \| H \land \yK) \;.
\\\text{for some logically independent data $D, D'$}\;.
\end{multline}
...
*** non-learning = independent

learning brings hyp1 = hyp2 + data (remark on Bayes factors Berger)

learning tells us how observed data influence beliefs about unobserved
