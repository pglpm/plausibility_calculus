...
%% Last-Updated: 2020-05-29T16:03:14+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
\emph{partially exchangeable}, that is, the $\set{\X{i}}$ can be divided
into two or more groups $\set{\Y{j}}, \set{\Z{k}}, \dotsc$, and
permutations are allowed within each group but not necessarily across
groups. The formula then becomes
\begin{multline}
...
\item there is some other quantity $C$ that allows us to distinguish the
  two groups, and \item the values $\C{i}\=\cc{i}$ of this quantity
...
result for a $\xj$uvenile patient, and $Z$ for an $\xa$dult patient. If we
write
$$\p(\Y{3}\=\xs,\;\Z{5}\=\xf \| I)=0.2\;,$$
then we must already know that patient number~$3$ is juvenile,
$\C{3}\=\xj$, and patient number~$5$ is adult, $\C{5}\=\xa$. This is clear
from our very notation, otherwise we would not have known whether to use
$Y$ or $Z$ for these patients. This information is evidently implicit in
our background information $I$.

Let us make the information about $C$ explicit. Its values can be
\enquote{$\xY$} and \enquote{$\xZ$}. We rewrite the probability in
\eqn~\eqref{eq:partialdefinetti} as
\begin{equation}
...
\item they have, as pairs, a fully exchangeable probability distribution,
\item the distribution for the $\set{\X{i}}$ is only conditionally
  exchangeable, given $\set{\C{i}}$.
\end{enumerate}
The formula is:
...
\item $\bigl(f_{x \bcond c} \defd f_{x, c}/f_{,c} \bigr)$ are the related
  \emph{conditional} frequency distributions of $x$ given $c$, obtained
  from $\ff$.
\end{itemize}

\smallskip

The noteworthy feature of the formula above is that the conditional
exchangeability for $\set{\X{i}}$ given $\set{\C{i}}$ is expressed by \emph{the
  factorizability of the density $\pf(\ff\|I)$} into
a density for the conditional frequencies, and a density for the marginal
frequencies, \eqn~\eqref{eq:factorizable}.
